...
dif = theSimulator.createEntity('DiffusionProcess', 'Process:/Surface:diffhemiBand3')
dif.VariableReferenceList = [['_', 'Variable:/Surface:hemiBand3']]
dif.D = 1.0e-14

dif = theSimulator.createEntity('DiffusionProcess', 'Process:/Surface:diffhemiBand3oxi')
...
dif = theSimulator.createEntity('DiffusionProcess', 'Process:/Surface:diffhemiBand3phos')
...
# # binder = theSimulator.createEntity('SpatiocyteNextReactionProcess', 'Process:/Surface:declusteroxi')
# # binder.VariableReferenceList = [['_', 'Variable:/Surface:Band3cluster','-1'],
# #                                 ['_', 'Variable:/Surface:Band3oxi',    '1']]
# # binder.k = 1e-3

# ### reactions2 ###
# # binder = theSimulator.createEntity('SpatiocyteNextReactionProcess', 'Process:/Surface:hemi')
# # binder.VariableReferenceList = [['_', 'Variable:/:hemi',            '-1'],
# #                                 ['_', 'Variable:/Surface:Band3',    '-1'],
# #                                 ['_', 'Variable:/Surface:hemiBand3', '1']]
# # binder.k = 1e-23
...
with open(log_file, 'a') as output :
    output.write(str(getCurrentTime()) + '\t' +
                 str(GSH.Value)        + '\t' +
                 str(Dia.Value)        + '\t' +
                 str(Band3.Value)      + '\t' +
...
while getCurrentTime() < duration + duration_beforediamide :
    run(1)
    with open(log_file, 'a') as output :
        output.write(str(getCurrentTime() - duration_beforediamide) + '\t' +
                     str(GSH.Value)        + '\t' +
                     str(Dia.Value)        + '\t' +
                     str(Band3.Value)      + '\t' +
                     str((Band3oxi.Value) + (hemiBand3oxi.Value) + (Band3phos.Value) + (Band3cluster.Value) + (hemiBand3phos.Value) + (hemiBand3cluster.Value))   + '\t' +
